        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
                <form action="{{ route('admin.rekap-penjualan.export') }}" method="POST" class="flex items-center gap-2">
                    @csrf
                    <select name="period"
                        class="block w-full pl-3 pr-10 py-2 text-sm border-slate-200 focus:outline-none focus:ring-brand-pink focus:border-brand-pink rounded-xl">
                        <option disabled selected>Pilih Periode</option>
                        <option value="week">1 Minggu Terakhir</option>
                        <option value="month">1 Bulan Terakhir</option>
                    </select>
                    <button type="submit"
                        class="inline-flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white text-sm font-semibold rounded-xl hover:bg-emerald-700 transition-all shadow-sm">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-100">
                            ...
                            <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center">Qty
                            </th>
                            <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">Harga
                                Item</th>
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="px-6 py-4">
                                    ...
                                    <div class="text-[10px] text-slate-400 mt-0.5">
                                        {{ $detail->order->delivery->spb_number ?? '-' }}</div>
                                </td>
                                <td class="px-6 py-4">
